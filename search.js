const scholarshipGrid = document.getElementById('advancedScholarshipGrid');
...
function renderScholarships(data) {
  ...
  if (!data || data.length === 0) {
    scholarshipGrid.innerHTML = `
      <div style="grid-column: 1/-1; text-align: center; padding: 4rem; color: var(--text-muted); background: var(--card-bg); border-radius: 12px; border: 1px solid var(--border);">
  ...
  data.forEach(item => {
    ...
    // Add verified badge if it came from grants.gov
...
async function performSearch() {
  ...
  scholarshipGrid.innerHTML = `
    <div style="grid-column: 1/-1; text-align: center; padding: 4rem;">
      ...
      <h3 style="margin-top: 1rem;">Scanning live databases...</h3>
      <p style="color: var(--text-muted);">Fetching data from Grants.gov and AI engines.</p>
    </div>
  `;
